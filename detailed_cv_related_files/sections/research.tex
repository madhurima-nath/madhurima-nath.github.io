...
\begin{zitemize}
\item Developed a hybrid computational framework combining Monte Carlo simulations with weak- and strong-coupling perturbative expansions to estimate Moore-Shannon network reliability on graphs, an NP-hard challenge.
[{\ref{arxiv-paper}}]

\item Extended these statistical physics methods to heterogeneous probabilistic satisfiability, constructing provably tighter upper and lower bounds on approximation error for monotonic problem instances. [{\ref{arxiv-paper}}]

\item Implemented these approaches for community detection in large-scale weighted directed networks, characterising clusters with respect to underlying dynamical processes to overcome the limitations of traditional modularity-based approaches.
[{\ref{com-det}}]
\end{zitemize}
...
Analysis of National Longitudinal Study of Adolescent to Adult Health (Add Health) dataset demonstrated that models matching local network statistics can overestimate infection numbers by $\sim$50\%. Applied Birnbaum importance measures to quantify individual edge contributions to epidemic potential, enabling identification of critical connections for targeted mitigation strategies. [{\ref{epi-paper}}]

\item Identified vulnerabilities within global food trade networks using commodity-specific data from UN Comtrade database, analysing where pest and pathogen contamination could cascade through global supply chains. Achieved $\sim$96\% precision in forecasting impact of mitigation strategies under various contagion scenarios. [{\ref{trade}}]
...
\item Generated transmission spectra across Mott Insulator and Superfluid phases, establishing a non-destructive approach for probing Fock-space structure in few-body correlated quantum systems.
